...
*-------------------------------------------------------------------------------------------
log using "preliminary_results/prelim_results_PD", replace
*-------------------------------------------------------------------------------------------

use "Stata/data/PD_data_indiv", clear

*-------*ANALYSES PD* DATA UNIT=INDIVIDUALS*-------*
...
ranksum payoff if  treat==0|treat==1, by(treatment) // payoff solo > choice!!!

ranksum payoff if  treat==0|treat==2, by(treatment) // payoff rating = solo

ranksum payoff if  treat==1|treat==2, by(treatment) // payoff choice << rating !!
...
graph export "Stata/graphs/graph_payoff_pd.pdf", replace
...
graph export "Stata/graphs/graph_exploration_cost_pd.pdf", replace


*-------*ANALYSES PD* DATA UNIT=GROUPS*-------*

use "Stata/data/PD_data_group", clear
...
table treatment , c( mean correct_at_40 semean correct_at_40) // Correct_at_40 (1/0) represents the mean percentage of subjects in a group being correct (when this is equal or above 0.75 it corresponds to group_correct_at_40=1)

tobit correct_at_40 i.treatment, ll(0) ul(1) // 
...
graph export "Stata/graphs/graph_group_performance_pd.pdf", replace
...
graph export "Stata/graphs/graph_group_performance_1_pd.pdf", replace
...
twoway (scatter first_cb_round confidence if overconfidence==-1)(scatter first_cb_round confidence if overconfidence==1) if confidence<3 , by(treatment, note("") legend (row(1)) graphregion(color(white))) ytitle("decision time -n rounds-") xtitle("confidence ratio") legend(order(2 "overconfident" 1 "underconfident")) legend (row(1) region(lstyle(none))) graphregion(color(white))
graph export "Stata/graphs/graph_decision_time_pd.pdf", replace
...
* CHECK IF TOTAL EXPLORATION LEVELS ARE DIFF
ranksum fullexploration if treat==3|treat==0, by(treatment) // MAS=solo

ranksum fullexploration if  treat==3|treat==1, by(treatment) // MAS=choice

ranksum fullexploration if  treat==3|treat==2, by(treatment) // MAS=rating
...
tobit correct_at_40 ib4.treatment, ll(0) ul(1) // average % of subjects choosing the correct option within groups at 40 round
tobit correct_at_40 ib4.treatment if treatment !=0, ll(0) ul(1) // average % of subjects choosing the correct option within groups at 40 round
...
logit group_correct_at_40 ib3.treatment confidence_level , robust // including confidence

*translate file into pdf
graphlog using "preliminary_results/prelim_results_PD.smcl",  lspacing(0.8) fwidth(0.6) enumerate replace // to include graphs in pdf!
